...
Our blog is now running on [Astro][astro], which makes it much easier for us to post and handle the repo.

Our documentation has changed also to [nimbus-docs][nimbus-docs] (based on Astro). This move offered a much easier way to handle versions and redirects, which were sore points in our deployment process. A lot of the versioning and redirections are now parameter based, which helps us with new deployments.

Finally, one of our ideas is to create a `benchmarks` repository, which will compare Phalcon against other frameworks. The setup will be the one that TechEmpower used to use. Right now it is just a thought and investigation, because in order to be unbiased, we have to have that repository open and
transparent, and also to provision hardware that will not benefit one framework over another. Work in progress but we think we can do it.
...
[nimbus-docs]: https://docs.nimbus.dev/
