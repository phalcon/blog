...
- Fixed the Volt parser raising `Syntax error, unexpected token DEFAULT` when the `default` filter (e.g. `{% raw %}{{ value|default('text') }}{% endraw %}`) is used inside a `{% raw %}{% switch %}{% endraw %}` block. The word `default` is now treated as the `{% raw %}{% default %}{% endraw %}` clause only when it directly follows the opening `{% raw %}{%{% endraw %}` delimiter inside a switch; everywhere else it is parsed as a plain identifier, so the filter, `{% raw %}{{ default }}{% endraw %}` and `{% raw %}{% set default = ... %}{% endraw %}` all work inside switch-case blocks. [#16003](https://github.com/phalcon/cphalcon/issues/16003)
