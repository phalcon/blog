...
- Added parameter that changes the default operator for conditions in method `Mvc\Model\Criteria::fromImput()` [GI:10749]
...
- Add `CookieInterface`, update `Cookie` and `Cookies` to use this interface - Decoupling `Cookies` and `Cookie` - Check `Session` state before using it in `Cookie`. [GI:10789]
...
- When creating tables in Postgres, inline PRIMARY keys are now escaped properly [GI:10797]
