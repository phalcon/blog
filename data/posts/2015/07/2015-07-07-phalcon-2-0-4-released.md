...
As part of our three-five weeks minor release schedule, we are excited to announce that
Phalcon 2.0.4 has been released!

The number of improvements and bug fixes is bigger compared to other releases
in the 2.0.x series:
...
Before this version, only standard placeholders (strings and numerical) were supported in PHQL [PHQL](https://docs.phalconphp.com/en/latest/reference/phql.html). Placeholders allowed you to bind parameters
...
However, some database systems require additional actions when using placeholders
like specify the bind type:
...
Typed placeholders are also more powerful, you can bind a static array
...
This happens because 100 is a string variable, you can easily fix this by casting the value to integer first:
...
However this solution requires that the developer pay special attention about how bound parameters are passed
and their types. To make this task easier and avoid unexpected exceptions you can instruct Phalcon to do this cast
for you:
...
by PDO, no matter if the value belongs to a numerical or boolean column.
This happens because some kind of columns cannot be represented with its corresponding
PHP native type due to the size limits of them.

For instance, a BIGINT in MySQL can store large integer numbers that cannot be represented as
a 32bit integer in PHP. So PDO, and the ORM by default, makes the safe decision of
leaving all values as strings.

However, some developers may find this unexpected and uncomfortable.
From Phalcon 2.0.4, you can set up the ORM to automatically cast those types considered safe
to the corresponding PHP native types:
